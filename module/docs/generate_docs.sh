...
usage() {
  cat - >&2 <<EOF
NAME
    generate_docs.sh - Automate terraform documentation

SYNOPSIS
    generate_docs.sh [-h|--help]
    generate_docs.sh [-d|--dir[=<arg>]]
...
initialize(){
  ...
  TEMPLATE_MAIN_DOC=("docs/main_doc_project.md")
}
...
# Append main readme section
append_main_project_doc(){
  pushd "$DIR" >/dev/null

  if [ -f "${TEMPLATE_MAIN_DOC}" ];
    then
        echo "Generating TEMPLATE_MAIN_DOC template ................."
        echo

        echo -e "$(cat "$TEMPLATE_MAIN_DOC")\n\n" >>  "$README_NEW"

        echo "Appended TEMPLATE_MAIN_DOC template."
        echo
    else
        echo "Error, TEMPLATE_MAIN_DOC file  --> $TEMPLATE_MAIN_DOC is not present in folder docs/ examined in path $(pwd)"
        exit 3
  fi
